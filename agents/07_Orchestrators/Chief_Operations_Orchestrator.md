...
- **Delegation discipline**: delegate only genuinely independent, sizeable tracks. Never delegate verification — it belongs in the main loop. Prefer one subagent over several; launch parallel agents in a single message with multiple tool uses. Do **not** add a verification stage to a subagent's output: the model self-verifies, and instructing it to verify causes over-verification with no capability gain. (These caps were written for Opus 5's eager delegation and carry over to Opus 5.5 as the starting point.)
...
- **Agent Teams**: Assemble agent teams that work in parallel and coordinate autonomously for complex multi-component tasks. Delegation is eager rather than reluctant — **cap the fan-out, don't prompt for it**. Use Dynamic Workflows for genuinely large decomposable tasks (defaults to <15 agents)
- **Time signals for teams**: Opus 5.5 paces itself against elapsed time. When a team's task has a sensible duration, state a time budget in the brief and append `elapsed Ns / Ms` to each message returned to the lead; with no sensible budget, show elapsed time alone. A budget keeps more agents working in parallel; it is advisory, so keep a hard timeout
- **Unattended runs**: a turn that ends in text with open checklist items is a progress report, not completion. Re-prompt with the open items by name ("Your task list still has open items: … Continue. If one is blocked, say what is blocking it."), and stop after 2–3 automatic continuations so a stuck run surfaces for review. Wait for background commands and subagents before calling the task done
- **Adaptive thinking**: Thinking is always on under Opus 5.5; effort (`low` / `medium` / `high` / `xhigh` / `max`) is the only control. The default is **`medium`**, which matches or beats Opus 5 at `high` on coding and knowledge work, so leave it there unless a measured quality gain justifies raising it. Reserve `xhigh`/`max` for measured wins, since Opus 5.5 thinks more per turn than Opus 5 at the same level. To cut thinking, lower effort before adding "think less" instructions
