...
**35 custom skills** + 9 official Anthropic skills with progressive disclosure (3-level loading). Framework specialists preload matching skills via `skills:` frontmatter. See [skills-integration.md](docs/rules/skills-integration.md) and [SKILLS.md](SKILLS.md).
...
`pipeline-quality`, `pipeline-review`, and `pipeline-full-build` skills provide universal, stack-adaptive pre-merge and release automation. `pipeline-quality` is the **deterministic CI gate** (lint, type check, Semgrep SAST, tests, happy/non-happy/edge **test case matrix**, dead-code, dep audit); `pipeline-review` is its **reasoning counterpart** (behavior-preserving simplification → severity-rated correctness/security review → Conventional Commit, delegated to the `code-reviewer` agent); `pipeline-full-build` is the **end-to-end release chain** — 7 phases / 17 steps: Safeguard (verified failsafe backup) → Verify → Integrate (bump, commit, merge, push) → Build → Ship (release, deploy to prod, rollback gate) → Document (project docs, then Claude docs) → Reclaim (local + VPS junk, dangling images). Its safety contract: nothing is deleted before it is backed up, nothing ships before it is verified, nothing is reclaimed before the release is confirmed healthy — and at least one verified backup always survives.

Phase 3 and Step 12 are **delegated to a variant skill**, because desktop and cloud diverge in compilation, tests, validation, and what "production" even means. `pipeline-full-build-desktop` compiles locally against the **Electron ABI** (native module rebuilds, frozen Python sidecars), signs and notarizes, then validates the **packaged binary** — launch smoke, clean first-run, offline start, cross-platform matrix — and ships via a staged update feed that cannot be rolled back server-side. `pipeline-full-build-cloud` builds a container with SBOM and a CVE gate, validates against a **restored production schema** (migration up/down/up, lock inspection) plus contract and load tests on staging, then deploys canary→rolling with one-command rollback kept real by expand-only migrations. The parent routes on stack detection; invoke a variant directly when the target is known. `/feature-workflow` (Phases 5–6) and `/pr-ready` delegate their simplify+review step to `/pipeline-review` rather than inlining checklists, so review logic lives in one place. Preloaded on `code-reviewer` (all three review skills), `qa-engineer`, and `devops-engineer`.
